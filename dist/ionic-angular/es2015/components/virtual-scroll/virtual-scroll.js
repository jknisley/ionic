...
export class VirtualScroll {
    ...
    writeUpdate() {
        (void 0) /* console.debug */;
        const /** @type {?} */ stopAtHeight = ((this._data.scrollTop || 0) + this._data.renderHeight);
    ...
    scrollUpdate(ev) {
        ...
        if (this._queue === SCROLL_QUEUE_DOM_WRITE) {
            ...
            if (Math.abs(data.scrollDiff) > SCROLL_DIFFERENCE_MINIMUM) {
                ...
                adjustRendered(cells, data);
                var /** @type {?} */ hasChanges = populateNodeData(data.topCell, data.bottomCell, data.viewWidth, data.scrollDiff > 0, cells, this._records, nodes, this._itmTmp.viewContainer, this._itmTmp.templateRef, this._hdrTmp && this._hdrTmp.templateRef, this._ftrTmp && this._ftrTmp.templateRef, false);
                if (hasChanges) {
